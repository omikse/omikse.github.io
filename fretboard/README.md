...
- **The frequency ruler** answers the question a beginner actually has: what is the
  difference between a note and a scale. A note name is a *place* — C is 261.63 Hz, C♯ is
  277.18 Hz, and that number is the whole of what the name carries. A scale is a *pattern of
  gaps* laid over that line, and the panel draws both: the twelve semitones as ticks, the
  scale's notes as pins, and a labelled bracket over every gap (semitones, ratio, or hertz).
  Slide the root and the pattern travels along the axis without changing shape — the neck
  above follows, because that is the same move as playing the shape in another key. Turn on
  the second row and two scales share one root: same starting frequency, different spacing,
  and the first disagreement usually arrives at the third note.
  The axis toggle is the other half of the point. On **equal ratios** every semitone is the
  same width, because the axis multiplies (×1.0595 a step, twelve of them exactly ×2) — this
  is the axis the ear uses and the one the fretboard is cut to. On **equal hertz** the same
  scale goes lopsided, the bottom octave collapsing into the left third of the line, because
  a musical distance is a multiplication and a ruler that only adds cannot show it evenly.
  That is also, in one picture, why the frets crowd together as you climb.
...
the harmonics, the notation and the frequency ruler.

The ruler keeps its own state, and deliberately owns no root of its own:

```js
var FR = {
  axis:'log'|'lin', gaps:'semi'|'ratio'|'hz', oct:1|2|3,
  rootMidi:57, cmp:false, cmpScale:'major', pins:[]
};
```

`PL.root` stays the single source of truth for *which* root; `FR.rootMidi` only remembers
*which octave of it* to draw, and `drawRuler()` snaps itself to the nearest octave of
`PL.root` on every paint, so the panel cannot drift out of step with the rest of the view no
matter which control moved the root — a select, a share link, a guided session. Moving the
slider goes the other way: it sets `PL.root` and calls `renderPlay()`, which is why the neck
slides along with the pattern. `FR` is declared with `var` rather than `const` so that a
`renderPlay()` firing before this block is parsed cannot trip a temporal-dead-zone error.
